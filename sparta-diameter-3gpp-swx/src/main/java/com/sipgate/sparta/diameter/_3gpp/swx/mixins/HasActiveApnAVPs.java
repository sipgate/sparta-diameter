...
public interface HasActiveApnAVPs extends AVPContainer {
    ...
    default void addActiveApn(final List<AVP> avps) {
        addAVP(AVP.create(new AVPKey(_3gppConstants.AVP_ACTIVE_APN, _3gppConstants.VENDOR_ID_3GPP), avps));
    }

    default List<AVPContainer> getActiveApns() {
        final List<AVPContainer> result = new ArrayList<>();
        for (final AVP avp : findAVPs(new AVPKey(_3gppConstants.AVP_ACTIVE_APN, _3gppConstants.VENDOR_ID_3GPP))) {
