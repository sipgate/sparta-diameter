...
public interface HasSipAuthDataItemAVPs extends AVPContainer {
    ...
    default void addSipAuthDataItem(final List<AVP> avps) {
        addAVP(AVP.create(new AVPKey(_3gppConstants.AVP_SIP_AUTH_DATA_ITEM, _3gppConstants.VENDOR_ID_3GPP), avps));
    }

    default List<AVPContainer> getSipAuthDataItems() {
        final List<AVPContainer> result = new ArrayList<>();
        for (final AVP avp : findAVPs(new AVPKey(_3gppConstants.AVP_SIP_AUTH_DATA_ITEM, _3gppConstants.VENDOR_ID_3GPP))) {
    ...
    default void addAllSipAuthDataItems(final Collection<List<AVP>> values) {
        for (final List<AVP> avps : values) {
            addSipAuthDataItem(avps);
        }
    }
}
