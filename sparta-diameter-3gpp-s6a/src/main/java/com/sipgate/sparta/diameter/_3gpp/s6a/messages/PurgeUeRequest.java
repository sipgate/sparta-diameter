...
/**
 * Purge-UE-Request (PUR) — 3GPP TS 29.272 §7.2.13.
 */
public interface PurgeUeRequest
    ...
    final class In extends IncomingRequest<PurgeUeAnswer.Out>
            implements PurgeUeRequest {

        In(final HopByHopId hopByHop, final EndToEndId endToEnd, final boolean retransmitted) {
            super(S6aConstants.CMD_PURGE_UE, true, retransmitted,
                  S6aConstants.APP_ID_S6A_S6D, hopByHop, endToEnd);
        }
    }

    final class Out extends OutgoingRequest<PurgeUeAnswer.In>
            implements PurgeUeRequest {
