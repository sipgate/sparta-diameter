...
/**
 * Authentication-Information-Request (AIR) — 3GPP TS 29.272 §7.2.5.
 */
public interface AuthenticationInformationRequest
    ...
    final class In extends IncomingRequest<AuthenticationInformationAnswer.Out>
            implements AuthenticationInformationRequest {

        In(final HopByHopId hopByHop, final EndToEndId endToEnd, final boolean retransmitted) {
            super(S6aConstants.CMD_AUTHENTICATION_INFORMATION, true, retransmitted,
                  S6aConstants.APP_ID_S6A_S6D, hopByHop, endToEnd);
        }
    }

    final class Out extends OutgoingRequest<AuthenticationInformationAnswer.In>
            implements AuthenticationInformationRequest {
